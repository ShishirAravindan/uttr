...
/// and keeps @Published status in sync so SwiftUI views reflect it live. While a
/// permission is still missing, it polls every 1.5s and also refreshes on
/// app-activation, so granting in System Settings takes effect immediately —
/// no quit-and-relaunch required.
class PermissionManager: ObservableObject {
    ...
    /// Requests both permissions in a coherent first-run flow:
    ...
    /// Drives the primary button for a permission row in Settings.
    ...
    /// Updates accessibility status and fires a notification when it transitions to
    /// authorized, so the app can re-register global hotkeys without restarting.
